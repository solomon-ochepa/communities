            ...
            <div class="card-body p-3 rounded shadow bg-secondary">
                ...
                <section class="row g-3 _mb-3 _pb-3">
                    <div class="col-md-12">
                        ...
                        <input type="search" class="form-control" id="search-user" value="{{ old('search') }}"
                            placeholder="" wire:model="search">
                        <div class="form-text">
                                    ...
                                    <section class="row gy-3 pb-md-0 pb-2">
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="date" value="{{ $user['moved_in'] ?? '' }}"
                                                    ...
                                                    @error("apartments.{$apartment->id}.residents.{$user['id']}.moved_in") is-invalid @enderror
                                                    wire:model.lazy="apartments.{{ $apartment->id }}.residents.{{ $user['id'] }}.moved_in"
                                                    aria-label="Moved in" aria-describedby="room-icon" _required />
                                                @error("apartments.{$apartment->id}.residents.{$user['id']}.moved_in")
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" value="{{ $user['room'] ?? '' }}" id="room"
                                                    list="roomList" class="form-control" placeholder=""
                                                    @error("apartments.{$apartment->id}.residents.{$user['id']}.room") is-invalid @enderror
                                                    wire:model.lazy="apartments.{{ $apartment->id }}.residents.{{ $user['id'] }}.room"
                                                    aria-label="Room" aria-describedby="room-icon" _required />
                                                <datalist id="roomList">
                                                    @foreach ($apartment->rooms as $room)
                                                        <option value="{{ $room->name }}">
                                                    @endforeach
                                                </datalist>
                                                @error("apartments.{$apartment->id}.residents.{$user['id']}.room")
                                                    <div class="form-text text-danger error">
                                                        {{ $message }}
                                                    </div>
                                                @enderror
