                                            ...
                                            @if ($attendance->checkin_time)
                                                <span class="text-success">
                                                    ...
                                                    @if ($attendance->checkout_time)
                                                        <span class="text-danger ml-2"> {{ __('attendance.clock_out_at') }}
                                                            -
                                                            {{ $attendance->checkout_time }}</span>
                                                    @endif
                                                </span>
                                            @endif
                                        </p>
                                        @if (!$attendance->checkout_time)
                                            <form action="{{ route('office.attendance.clockout') }}" method="post">
                                                ...
                                                <button class="btn  d-flex inputbtnclockout align-items-center btn-dark"
                                                    type="submit"><i
                                                        class="fas fa-4x fa-sign-out-alt"></i>{{ __('attendance.clock_out') }}</button>
                                            </form>
                                        @endif
                                    </div>
                                @else
                                    <div class="float-right  d-flex text-center" style="margin-left:auto">
                                        ...
                                        <button type="button"
                                            class="btn  d-flex inputbtnclockin align-items-center btn-success"
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover" id="maintable"
                                    data-url="{{ route('office.attendance.get-attendance') }}">
                                    <thead>
                                        <tr>
                                            <th>{{ __('levels.id') }}</th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ __('attendance.clock_in') }} - <span
                ...
                <form action="{{ route('office.attendance.clockin') }}" method="POST">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary"
                            data-dismiss="modal">{{ __('attendance.close') }}</button>
