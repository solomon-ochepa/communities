        ...
        <div class="section-body">
            <div class="row">
                <div class="col-12 col-md-12 col-lg-12">
                    <form action="{{ route('office.user.update', $user) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col">
                                        <label for="role_id">{{ __('administrator.role') }}</label> <span
                                            class="text-danger">*</span>
                                        <select id="role_id" name="role_id"
                                            class="form-control @error('role_id') is-invalid @enderror">
                                            @foreach ($roles as $key => $role)
                                                <option value="{{ $role->id }}"
                                                    {{ old('role_id', $user->myrole) == $role->id ? 'selected' : '' }}>
                                                    {{ $role->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('role_id')
                                    ...
                                    <div class="form-group col">
                                        ...
                                        <div class="custom-file">
                                            ...
                                            <label class="custom-file-label"
                                                for="customFile">{{ __('administrator.choose_file') }}</label>
                                        </div>
                                        @if ($errors->has('image'))
...
@section('scripts')
    <script src="{{ asset('js/office/user/edit.js') }}"></script>
@endsection
