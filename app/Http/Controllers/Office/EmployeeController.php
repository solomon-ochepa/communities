...
class EmployeeController extends Controller
{
    ...
    public function show($id)
    {
        ...
        return view('office.employee.show', $this->data);
    }
        ...
        return Datatables::of($visitorArray)
            ->addColumn('action', function ($visitor) {
                ...
                if (auth()->user()->can('pre-register.show')) {
                    $retAction .= '<a href="' . route('office.visitors.show', $visitor) . '" class="btn btn-sm btn-icon mr-2  float-left btn-info" data-toggle="tooltip" data-placement="top" title="View"><i class="far fa-eye"></i></a>';
                }

                if (auth()->user()->can('pre-register.edit')) {
                    $retAction .= '<a href="' . route('office.visitors.edit', $visitor) . '" class="btn btn-sm btn-icon float-left btn-primary" data-toggle="tooltip" data-placement="top" title="Edit"> <i class="far fa-edit"></i></a>';
                }


                if (auth()->user()->can('pre-register.delete')) {
                    $retAction .= '<form class="float-left pl-2" action="' . route('office.visitors.destroy', $visitor) . '" method="POST">' . method_field('DELETE') . csrf_field() . '<button class="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"> <i class="fa fa-trash"></i></button></form>';
                }
    ...
    public function getPreRegister($id)
    {
        ...
        return Datatables::of($pre_registerArray)
            ->addColumn('action', function ($pre_register) {
                ...
                if (auth()->user()->can('pre-register.delete')) {
                    $retAction .= '<form class="float-left pl-2" action="' . route('office.pre-registers.destroy', $pre_register) . '" method="POST">' . method_field('DELETE') . csrf_field() . '<button class="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"> <i class="fa fa-trash"></i></button></form>';
                }
