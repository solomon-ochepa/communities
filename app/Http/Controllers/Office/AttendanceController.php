...
class AttendanceController extends BackendController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('office.attendance.index', compact('attendance'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('office.attendance.index')->withSuccess('The data delete successfully!');
    }
        ...
        return Datatables::of($attendanceArray)
            ->addColumn('action', function ($attendance) {
                ...
                if (auth()->user()->can('attendance.delete')) {
                    $retAction .= '<form class="float-left  " action="' . route('office.attendance.destroy', $attendance) . '" method="POST">' . method_field('DELETE') . csrf_field() . '<button class="btn btn-sm btn-icon btn-danger" data-toggle="tooltip" data-placement="top" title="Delete"> <i class="fa fa-trash"></i></button></form>';
                }
