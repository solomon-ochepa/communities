...
class AddonController extends BackendController
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:addon'])->only('index');
    ...
    public function index()
    {
        ...
        return view('office.addon.index', $this->data);
    }

    public function create()
    {
        if (env('DEMO')) {
            return redirect(route('office.addons.index'))->withError('Addon upload system is disable for the demo');
        }
        return view('office.addon.create', $this->data);
    }
    ...
    public function store(AddonRequest $request)
    {
        ...
        if ($response->status) {
            ...
            return redirect(route('office.addons.index'))->withSuccess('Addon install successfully.');
        } else {
            return redirect(route('office.addons.index'))->withError($response->message);
        }
    }
    ...
    public function destroy($id)
    {
        ...
        if ($file_response->status) {
            $addon->delete();
            return redirect(route('office.addons.index'))->withSuccess('The data deleted successfully.');
        } else {
            return redirect(route('office.addons.index'))->withError($file_response->message);
        }
    }
