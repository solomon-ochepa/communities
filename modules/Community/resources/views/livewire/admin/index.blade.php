                    ...
                    <tr>
                        <td>
                            <input type="checkbox" name="selected" id="selected"
                                wire:model.defer="selected.{{ $key }}.{{ $community->id }}" />
                        </td>
                        <td>
                            <a href="{{ route('admin.apartment.show', ['apartment' => $community->id]) }}">
                                {{ $community->name }}
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('admin.apartment.room.index', ['apartment' => $community->id]) }}">
                                <i class="fas fa-home text-muted me-1"></i>
                                {{ $community->rooms->count() }}
                            </a>
                        </td>
                        <td>
                            <a href="{{ route('admin.apartment.occupant.index', ['apartment' => $community->id]) }}">
                                <i class="fas fa-users text-muted me-1"></i>
                                {{ $community->occupants->count() }}
                            </a>
                        </td>
                        <td>
                            @if ($community->active)
                        ...
                        <td class="text-end">
                            <div class="action-btns">
                                <a href="{{ route('admin.apartment.show', $community->id) }}"
                                    class="action-btn btn-view bs-tooltip me-2" data-toggle="tooltip"
                                    data-placement="top" title="View">
                                    <i class="fas fa-eye"></i>
                                </a>

                                <a href="{{ route('admin.apartment.edit', $community->id) }}"
                                ...
                                <form class="d-inline" method="POST"
                                    action="{{ route('admin.apartment.destroy', $community->id) }}">
                                    @method('delete')
                                    @csrf
